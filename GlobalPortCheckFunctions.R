...
datacompletion <- function (Data){
  ...
  Data <- subset(Data,!is.na(CompanyDomicileRegion) & (Sector == "Power" | (Sector == "Fossil Fuels" & BenchmarkRegion %in% c(AllLists$FossilFuelBenchmarkRegions,"Global")) | (Sector == "Automotive" & BenchmarkRegion == "Global")))
...
# logfiles
#------
initiateLogFile <- function(TypeofLogfile){
  logfile <- paste0(TypeofLogfile,"logfile.txt")
  cat("",file = logfile,append = FALSE)
}

addMessageToLogFile <- function(TypeofLogfile,LogfileMessage){
  logfile <- paste0(TypeofLogfile,"logfile.txt")
  if (!file.exists(logfile)){initiateLogFile(TypeofLogfile)}
  cat(paste0(TypeofLogfile,": ", LogfileMessage),file = logfile,append = TRUE, sep="\n")
}
